...
- GELU variant: `gelu()` (tanh approximation) → `gelu_erf()` (exact erf) in encoder conv stem and adapter. Systematic bias corrupted audio features, causing all-PAD decoder output.
