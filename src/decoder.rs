...
// ---- Dual-delay support: external decoder state ----

/// Externalized decoder state for dual-delay mode.
/// Holds per-session mutable state (KV caches, Ada-RMSNorm scales)
/// while TextDecoder holds shared immutable weights.
pub struct DecoderState {
    caches: Vec<KvCache>,
    ada_scales: Option<Vec<Tensor>>,
}

/// Dual-delay methods: shared weights (&self) + external mutable state (&mut DecoderState).
impl TextDecoder {
    /// Create a fresh decoder state (empty KV caches, no Ada scales).
    ...
    /// Reset an external state's KV caches.
    pub fn reset_state(state: &mut DecoderState) {
        for cache in &mut state.caches {
            cache.reset();
        }
    }

    /// Trim an external state's KV caches to the sliding window.
    pub fn trim_state(state: &mut DecoderState) {
        for cache in &mut state.caches {
            cache.trim(SLIDING_WINDOW);
        }
    }

    /// Precompute per-layer Ada-RMSNorm scales into an external state.
    pub fn precompute_t_cond_into(&self, t_cond: &Tensor, state: &mut DecoderState) -> candle_core::Result<()> {
        let scales = self.layers.iter()
            .map(|layer| {
                let s = layer.ada_norm.forward(t_cond)?;
                s.unsqueeze(1)? + 1.0
            ...
            .collect::<candle_core::Result<Vec<_>>>()?;
        state.ada_scales = Some(scales);
        Ok(())
    }

    /// Forward pass using external state. Shares weights with other DecoderState instances.
    pub fn forward_with_state(&self, inputs_embeds: &Tensor, state: &mut DecoderState) -> candle_core::Result<Tensor> {
        let mut x = inputs_embeds.clone();

        let ada_scales = state.ada_scales.as_ref()
            .expect("precompute_t_cond_into must be called before forward_with_state");

        for (i, layer) in self.layers.iter().enumerate() {
            x = layer.forward(&x, &self.rope, &mut state.caches[i], &ada_scales[i])?;
        }
